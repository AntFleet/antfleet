/**
 * One-off — apply migration 0014 (weekly_features table).
 * Mirrors apply-migration-0013.ts.
 *
 * Usage (from apps/web):
 *   pnpm exec tsx scripts/apply-migration-0014.ts            # dry-run
 *   pnpm exec tsx scripts/apply-migration-0014.ts --apply    # mutate
 */
...
const TAG = "0014_weekly_features";
    ...
    const existence =
      (
        ...
        SELECT EXISTS (
          ...
          WHERE table_schema = 'public' AND table_name = 'weekly_features'
        ) AS exists
      `)
      ).rows[0]?.exists ?? false;
    console.log(`[public.weekly_features] exists=${existence}`);
