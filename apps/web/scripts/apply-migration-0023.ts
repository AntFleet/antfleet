/**
 * One-off — apply migration 0023 (Eval Phase 0 dual-candidate persistence).
 *
 * Adds suggested_patch_opus, suggested_patch_gpt5, patch_shipped, patch_selector
 * to finding_status + backfills existing Opus-attributed rows.
 *
 * Usage (from apps/web):
 *   pnpm exec tsx scripts/apply-migration-0023.ts            # dry-run
 *   pnpm exec tsx scripts/apply-migration-0023.ts --apply    # mutate
 */
...
const TAG = "0023_eval_phase_0_dual_candidate";
    ...
    const cols = await pool.query<{ column_name: string }>(
      `SELECT column_name FROM information_schema.columns
       WHERE table_name = 'finding_status'
         AND column_name IN ('suggested_patch_opus', 'suggested_patch_gpt5', 'patch_shipped', 'patch_selector')`,
    );
    console.log(
      `[finding_status] new columns: ${cols.rows.length === 0 ? "none (migration needed)" : cols.rows.map((r) => r.column_name).join(", ")}`,
    );
