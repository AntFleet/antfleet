...
export async function generateMetadata({ params }: { params: Promise<RouteParams> }): Promise<Metadata> {
  ...
  return { title: agent === null ? "AntFleet · Agent not found" : `AntFleet · ${agent.name} drift` };
}
...
export default async function DriftPage({ params }: { params: Promise<RouteParams> }) {
  ...
  const [identity, snapshots] = await Promise.all([fetchIdentityFile(agent), loadDriftSnapshots(agent.address)]);
  const threshold = identity.ok ? identityThreshold(identity.parsed) : snapshots.at(-1)?.threshold ?? 0.25;
    ...
    <main className="mx-auto max-w-5xl px-6 py-16">
      ...
      <div className="mt-6 flex flex-col gap-6 sm:flex-row sm:items-end sm:justify-between">
        ...
        <div className="rounded-md border border-[var(--color-line)] px-4 py-3">
          ...
          <p className="mt-1 text-2xl font-semibold text-[var(--color-ink)]">{threshold.toFixed(2)}</p>
        </div>
      </div>
          ...
          {snapshots.length === 0 ? (
            ...
            snapshots.slice(-12).toReversed().map((snapshot) => (
              <div key={snapshot.commitSha} className="grid gap-2 py-4 text-sm sm:grid-cols-[1fr_120px_120px]">
                ...
                <span className="text-[var(--color-ink-subtle)]">{snapshot.commitTimestamp.toISOString().slice(0, 10)}</span>
              </div>
            ))
          )}
        </div>
...
function DriftChart({
  ...
  const points = snapshots.map((snapshot, index) => {
    const x = snapshots.length <= 1 ? pad : pad + (index / (snapshots.length - 1)) * (width - pad * 2);
  ...
  return (
    <svg viewBox={`0 0 ${width} ${height}`} className="h-auto w-full" role="img" aria-label="Identity drift line chart">
      <rect x="0" y="0" width={width} height={height} fill="#ffffff" />
      <line x1={pad} y1={thresholdY} x2={width - pad} y2={thresholdY} stroke="#d8d8de" strokeDasharray="6 6" />
      <text x={width - pad} y={Math.max(16, thresholdY - 8)} textAnchor="end" fontSize="11" fill="#8a8a94" fontFamily="monospace">
        threshold {threshold.toFixed(2)}
      </text>
      {points.length > 1 && <polyline points={line} fill="none" stroke="#0a0a0a" strokeWidth="2.5" />}
